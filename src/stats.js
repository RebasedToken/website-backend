...
async function get(intervals, getFromToForInterval, burntAmount) {
  ...
  for (i = 0; i < intervals; i++) {
    ...
    if (!(price.isZero() || supply.isZero())) {
      ...
      supplies.unshift(
        web3.utils.fromWei(supply.sub(burntAmount), 'gwei').toString()
      )
    }
  }
...
const getBurntAmount = (exports.getBurntAmount = function () {
  return contracts.rebasedV2.contract.read('balanceOf', [
    '0x000000000000000000000000000000000000dead',
  ])
})

exports.getTotalSupply = async function () {
  const a = await contracts.rebasedV2.contract.read('totalSupply')
  const b = await getBurntAmount()
  return bn(a).sub(bn(b)).toString()
}
